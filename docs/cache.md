...
```python
# this happens automatically inside evaluate() (evaluate.py)
cache = {}
...
`evaluate()` (in `evaluate.py`) uses `extract_k_requirements()` (also in `evaluate.py`) to discover all `k`/`n_neighbors` values from metric configs, then calls `prewarm_cache()` (in `evaluate.py`) to compute kNN once with `max(k)`:
