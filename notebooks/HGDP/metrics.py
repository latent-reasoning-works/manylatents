...
#     return geographic_preservation, geographic_preservation_medians, geographic_preservation_far, admixture_preservation_dists, admixture_preservation_dists_medians, admixture_preservation_dists_far


# def connected_components_diff_op(phate_operator):
#     A = phate_operator.diff_op - np.diag(phate_operator.diff_op)*np.eye(len(phate_operator.diff_op))
#     graph = nx.from_numpy_array(A) # graph is from diffusion operator, minus diagonal
#     return np.sort(np.array([len(k) for k in nx.connected_components(graph)]))[::-1]
