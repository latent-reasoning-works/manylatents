...
class MIOFlow(LightningModule):
    ...
    def _group_by_time(self, batch: dict) -> list[tuple[Tensor, float]]:
        ...
        Accepts the manyLatents op-contract key ``"label"`` (singular, as
        emitted by ``InMemoryDataset``/``PrecomputedDataModule``), the
        ``"labels"`` (plural) key used by MIOFlow's own prototype datamodules,
        and ``"time"`` (as emitted by ``manylatents.api.run()``'s generic
        pipeline). For MIOFlow the label *is* the per-sample timepoint.
        """
        if isinstance(batch, dict):
            data = batch["data"]
            labels = batch.get("labels", batch.get("label", batch.get("time")))
            if labels is None:
                raise KeyError(
                    "MIOFlow requires a per-sample time label in the batch under "
                    "'label', 'labels', or 'time'; got keys "
                    f"{sorted(batch.keys())}."
                )
        else:
