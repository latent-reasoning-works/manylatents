...
class AOUDataModule(LightningDataModule):
    """
    PyTorch Lightning DataModule for All of Us Biobank (AOU).
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        """
        Instantiate AOUDataset(s) depending on `self.mode`.
        ...
        if self.mode == "full":
            self.train_dataset = AOUDataset(data_split="full", **common_kwargs)
            self.test_dataset = self.train_dataset   # same object
        elif self.mode == "split":
            self.train_dataset = AOUDataset(data_split="train", **common_kwargs)
            self.test_dataset = AOUDataset(data_split="test", **common_kwargs)
        else:
