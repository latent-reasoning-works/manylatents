...
class PrecomputedMixin:
    """
    Mixin to handle logic for loading precomputed raw data for synthetic datasets.
    
    This is different from PrecomputedDataset which loads saved embeddings/outputs.
    This mixin is used to load pre-generated synthetic dataset raw data to avoid
    expensive re-computation.
    """
    def load_precomputed(self, precomputed_path: str, mmap_mode: str = None):
        """Load precomputed raw data from disk."""
        if precomputed_path:
            abs_path = os.path.abspath(precomputed_path)
            logger.info(f"Resolved precomputed path: {abs_path}")
            if os.path.exists(abs_path):
                logger.info(f"Loading precomputed data from {abs_path}")
        ...
        logger.info("No precomputed data found or path does not exist.")
        return None
